...
# The game falls back to keyboard-only if the library or port is unavailable.
try:
...
# ── Arduino / joystick settings ───────────────────────────────────────────────
SERIAL_PORT      = "COM3"        # Windows: "COM3", "COM4" …
#SERIAL_PORT     = "/dev/ttyUSB0"   # Linux
#SERIAL_PORT     = "/dev/tty.usbmodem14101"  # macOS
SERIAL_BAUD      = 9600
JOY_CENTER       = 512           # resting value of each axis (0-1023)
JOY_DEADZONE     = 150           # ± around centre that counts as neutral
JOY_FIRE_THRESH  = JOY_CENTER + JOY_DEADZONE   # Y > this → FIRE (pulled back)
JOY_LANE_CD_MAX  = 22            # frames between joystick-triggered lane shifts
...
C_ZOMBIE_N   = (60,  180,  60)
C_ZOMBIE_F   = (180,  60,  60)
C_ZOMBIE_T   = (80,   80, 180)
...
pygame.display.set_caption("🧟 Zombie Lane Defense  [Arduino Edition]")
...
#
#  Runs a background thread that continuously reads the serial line sent by the
#  Arduino sketch.  The game loop calls .get_axes() and .consume_btn_event()
#  every frame without blocking.
#
#  If pyserial is not installed or the port is unavailable, connected=False and
#  the game silently falls back to keyboard-only controls.
#
class JoystickController:
    """
    Non-blocking serial reader for the Arduino joystick bridge.

    Arduino sends one CSV line per ~20 ms:   "X,Y,BTN\\n"
      X   – raw ADC value 0-1023 for the horizontal axis
      Y   – raw ADC value 0-1023 for the vertical  axis
      BTN – 1 while the stick button is held, 0 otherwise
    ...
    def __init__(self, port: str = SERIAL_PORT, baud: int = SERIAL_BAUD):
        ...
        self._x          = JOY_CENTER   # latest X reading
        self._y          = JOY_CENTER   # latest Y reading
        self._btn        = 0            # current raw button state
        self._btn_event  = False        # rising-edge flag consumed by game loop

        if not _SERIAL_AVAILABLE:
            print("[Arduino] pyserial not installed – keyboard only.")
            return
        ...
        try:
            ...
            print(f"[Arduino] Could not open {port}: {exc}  – keyboard only.")

    # ── Background thread ─────────────────────────────────────────────────────

    def _read_loop(self):
        """
        Continuously reads lines from the serial port and updates shared state.
        ...
        while self.connected:
            try:
                ...
                with self._lock:
                    ...
                    self._y   = y
                    self._btn = btn
                    # Rising edge → set the event flag
                    if btn == 1 and prev_btn == 0:
                        self._btn_event = True
                prev_btn = btn
            except Exception:
                pass   # malformed line or serial hiccup – just skip

    # ── API used by the game loop ──────────────────────────────────────────────

    def get_axes(self) -> tuple[int, int]:
        """Return the latest (X, Y) joystick values (0-1023 each)."""
        with self._lock:
            return self._x, self._y

    def consume_btn_event(self) -> bool:
        """
        Returns True ONCE per physical button press (rising-edge detection).
        Clears the internal flag so it won't fire again until the next press.
        Use this for pause / unpause so a held button doesn't spam toggles.
        """
        with self._lock:
            ev = self._btn_event
    ...
    def btn_held(self) -> bool:
        """Returns True while the button is physically held down."""
        with self._lock:
            return self._btn == 1

    def close(self):
        """Cleanly close the serial port (called on exit)."""
        self.connected = False
...
class Player:
    ...
    def set_lane(self, idx):
        ...
        self.target_y = lane_cy(self.lane)
...
class WaveManager:
    ...
    def update(self, zombies):
        ...
        if (self.zombies_spawned >= self.zombies_this_wave and not zombies):
            self.in_break    = True
            self.break_timer = FPS * 4
        return None
...
def draw_hud(surf, score, player, wave_mgr, joy: JoystickController):
    ...
    for i in range(LANES):
        cy = lane_cy(i)
        lbl = font_sm.render(f"[{i+1}]", True,
                              (80,100,120) if i!=player.lane else (120,200,255))
        surf.blit(lbl, lbl.get_rect(midright=(BASE_X-32, cy)))

    # ── Arduino connection indicator (bottom-left) ─────────────────────────
    #  Green dot + "JOY" when the serial connection is live.
    #  Red  dot + "KB"  when running keyboard-only.
    if joy.connected:
        jx, jy = joy.get_axes()
        dot_col = (50, 230, 50)
        label   = f"JOY  X:{jx:4d} Y:{jy:4d}"
    else:
        dot_col = (180, 60, 60)
        label   = "KB ONLY"
    pygame.draw.circle(surf, dot_col, (10, HEIGHT - 10), 6)
    lbl = font_sm.render(label, True, dot_col)
    surf.blit(lbl, (22, HEIGHT - 22))
...
def draw_pause_overlay():
    ...
    Called every frame while the game is paused so the frozen frame shows
    underneath.
    """
    ...
    draw_text_shadow(screen, "PAUSED", font_big, (255, 220, 80),
                     WIDTH // 2, HEIGHT // 2 - 50, "midtop")
    draw_text_shadow(screen, "Joystick click  or  [P]  to resume",
                     font_sm, (200, 200, 200), WIDTH // 2, HEIGHT // 2 + 20, "midtop")
    pygame.display.flip()


# ─── SCREENS ──────────────────────────────────────────────────────────────────

def screen_menu(joy: JoystickController):
    while True:
        screen.fill(BG_DARK)
        for i in range(LANES):
            c = GRASS_DARK if i%2==0 else GRASS_LIGHT
        ...
        lines = [
            "──── KEYBOARD ────────────────────────",
            "[1–4]  Switch lane     [SPACE] Fire",
            "[P]  Pause / unpause   [ESC]  Quit",
            "",
            "──── ARDUINO JOYSTICK ────────────────",
            "Stick LEFT / RIGHT  →  Change lane",
            "Pull BACK (Y high)  →  Fire",
            "Click button        →  Pause / unpause",
            "",
            "Collect power-ups for special ammo!",
            "",
            "Press  ENTER  or  joystick button  to start",
        ]
        for j, l in enumerate(lines):
            draw_text_shadow(screen, l, font_sm, (180,200,180), WIDTH//2, 190+j*26, "midtop")

        # Show connection status on menu too
        status_col = (50,230,50) if joy.connected else (180,60,60)
        status_txt = f"Arduino: {'CONNECTED' if joy.connected else 'NOT FOUND  (keyboard only)'}"
        draw_text_shadow(screen, status_txt, font_sm, status_col, WIDTH//2, HEIGHT-30, "midtop")
        ...
        # Joystick button also starts the game
...
def run_game(joy: JoystickController):
    player        = Player()
    bullets       : list[Bullet]   = []
    zombies       : list[Zombie]   = []
    ...
    score         = 0
    powerup_timer = 0
    holding_fire  = False   # keyboard space
    paused        = False

    # ── Joystick state for the game loop ──────────────────────────────────────
    joy_lane_cd   = 0        # cooldown so a held stick doesn't spam lane changes
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                # Lane keys always available (even while paused)
        ...
        # ── JOYSTICK INPUT ────────────────────────────────────────────────────
        # Processed every frame regardless of pause so the button can un-pause.
        if joy.connected:
            jx, jy = joy.get_axes()

            # Button (rising edge) → toggle pause
            if joy.consume_btn_event():
                paused = not paused

            if not paused:
                # ── Lane switching via X-axis ──────────────────────────────
                # Cooldown prevents a held stick from switching 60 lanes/sec.
                if joy_lane_cd > 0:
                    joy_lane_cd -= 1
                else:
                    if jx < JOY_CENTER - JOY_DEADZONE:
                        # Stick pushed LEFT → move to the lane above
                        player.set_lane(player.lane - 1)
                        joy_lane_cd = JOY_LANE_CD_MAX
                    elif jx > JOY_CENTER + JOY_DEADZONE:
                        # Stick pushed RIGHT → move to the lane below
                        player.set_lane(player.lane + 1)
                        joy_lane_cd = JOY_LANE_CD_MAX

                # ── Fire via Y-axis (pull toward player = high value) ──────
                # Mirrors holding SPACE – fires every frame the stick is held back.
                if jy > JOY_FIRE_THRESH:
                    b = player.shoot()
                    if b:
                        bullets.append(b)

        # ── PAUSE: skip simulation but keep screen alive ───────────────────
        if paused:
            draw_pause_overlay()
            continue
        ...
        if wave_mgr.in_break:
            ...
            banner = font_big.render(
                f"Wave {wave_mgr.wave} cleared!  Next in {secs}s…", True, (255,220,80))
...
def main():
    # Attempt to connect to the Arduino once at startup.
    # Change SERIAL_PORT at the top of the file to match your system.
    joy = JoystickController(port=SERIAL_PORT, baud=SERIAL_BAUD)

    screen_menu(joy)

    while True:
        score, wave = run_game(joy)
