...
class SoundManager:
    ...
    def _build_all(self):
        ...
        # ── Background Music (dark zombie march — looping) ──
        def bg_music():
            # Build a ~6-second looping dark orchestral/industrial pattern
            dur = 6.0; t = self._t(dur); total = len(t)
            sig = np.zeros(total)

            # --- Bass pulse (four-on-the-floor kick-like) ---
            kick_times = [0.0, 0.75, 1.5, 2.25, 3.0, 3.75, 4.5, 5.25]
            for kt in kick_times:
                idx = int(kt * SR)
                kl = int(0.28 * SR)
                if idx + kl > total: kl = total - idx
                kt2 = np.arange(kl) / SR
                kick = 0.50 * np.sin(2*np.pi*(90 - 60*kt2/0.28)*kt2) * np.exp(-kt2*18)
                kick += 0.22 * (2*np.random.random(kl)-1) * np.exp(-kt2*55)
                sig[idx:idx+kl] += kick

            # --- Dark synth bass line ---
            bass_notes = [(0.0, 55), (1.5, 58), (3.0, 52), (4.5, 49)]
            for bt, bf in bass_notes:
                idx = int(bt * SR); nl = int(1.4 * SR)
                if idx + nl > total: nl = total - idx
                nt2 = np.arange(nl) / SR
                note = 0.22 * np.sin(2*np.pi*bf*nt2)
                note += 0.10 * np.sin(2*np.pi*bf*2*nt2)
                env2 = np.exp(-nt2*1.2)
                sig[idx:idx+nl] += note * env2

            # --- Distant horn / string stab (ominous) ---
            stab_times = [0.75, 2.25, 3.75, 5.25]
            for st in stab_times:
                idx = int(st * SR); sl2 = int(0.55 * SR)
                if idx + sl2 > total: sl2 = total - idx
                st2 = np.arange(sl2) / SR
                f0 = 110
                stab = 0.18 * np.sin(2*np.pi*f0*st2)
                stab += 0.10 * np.sin(2*np.pi*f0*1.5*st2)
                stab += 0.06 * np.sin(2*np.pi*f0*2*st2)
                trem = 1 + 0.08*np.sin(2*np.pi*5*st2)
                stab *= trem * np.exp(-st2*3.0)
                sig[idx:idx+sl2] += stab

            # --- Snare / clap hits ---
            snare_times = [1.5, 3.0, 4.5, 6.0-0.01]
            for snt in snare_times:
                idx = int(snt * SR)
                snl = int(0.14 * SR)
                if idx + snl > total: snl = total - idx
                if snl <= 0: continue
                snt2 = np.arange(snl) / SR
                snare = (2*np.random.random(snl)-1) * np.exp(-snt2*38) * 0.35
                snare += 0.12 * np.sin(2*np.pi*220*snt2) * np.exp(-snt2*45)
                sig[idx:idx+snl] += snare

            # --- Ambient low rumble (continuous tension) ---
            rumble = self._noise(dur, 0.07) * (0.5 + 0.5*np.sin(2*np.pi*0.4*t))
            sig += rumble
        ...
        s['bg_music'] = self._to_snd(bg_music())
        s['bg_music'].set_volume(0.40)
    ...
    def play_shoot(self, weapon="normal"):
        if not self._ok: return
        # Route SMG/sniper to dedicated sounds
        if weapon in ('rapid',):
    ...
    def start_bg_music(self):
        if not self._ok: return
        if self._music_ch is None or not self._music_ch.get_busy():
            self._music_ch = self._sounds['bg_music'].play(loops=-1)
    ...
    def stop_bg_music(self):
        if not self._ok: return
        if self._music_ch:
            self._music_ch.stop(); self._music_ch = None
...
# ─── PAUSE MENU ───────────────────────────────────────────────────────────────
class PauseMenu:
    BASE_OPTS=["RESUME","RESTART","QUIT"]
    REBIRTH_OPT="REBIRTH"
    COLOURS={"RESUME":(70,215,100),"RESTART":(255,200,55),"QUIT":(220,65,55),"REBIRTH":(255,140,30)}
    def __init__(self): self.selected=0; self.can_rebirth=False
    @property
    def options(self): return self.BASE_OPTS[:2]+([self.REBIRTH_OPT] if self.can_rebirth else [])+[self.BASE_OPTS[-1]]
    def _btn_rect(self,idx):
        opts=self.options; bw,bh=290,50; cy2=HEIGHT//2-20+idx*(bh+14)
...
def screen_menu(joy):
    ...
    while True:
        ...
        draw_text(screen,"ZOMBIE LANE DEFENSE",font_big,tc,WIDTH//2,72,"midtop")
        ...
        rows=[
            ...
            ("[F] Melee    [G] Grenade (2-lane)    [P] Pause    [ESC] Quit",(175,195,175)),("",C_BLACK),
...
def run_game(joy,gs):
    ...
    bg=get_bg()
    sfx.start_bg_music()   # begin dark background music
